...
class CartRepository:
    ...
    async def create(self, currency: str = "USD") -> Cart:
        row = CartORM(id=str(uuid.uuid4()), currency=currency)
        self._session.add(row)
        await self._session.commit()
        return Cart(id=row.id, currency=currency, items=[])

    async def get(self, cart_id: str) -> Cart | None:
        row = await self._session.get(CartORM, cart_id)
        return _to_domain(row) if row else None

    async def _require(self, cart_id: str) -> CartORM:
        row = await self._session.get(CartORM, cart_id)
        if row is None:
            raise CartNotFoundError(cart_id)
        return row

    async def add_item(self, cart_id: str, *, sku: str, quantity: int, unit_price: Money) -> Cart:
        row = await self._require(cart_id)
    ...
    async def remove_item(self, cart_id: str, sku: str) -> Cart:
        row = await self._require(cart_id)
