...
async def _get_cart_or_404(repo: CartRepository, cart_id: str) -> Cart:
...
async def create_cart(session: SessionDep) -> Cart:
    return await CartRepository(session).create()


@router.get("/{cart_id}", response_model=Cart)
async def view_cart(cart_id: str, session: SessionDep) -> Cart:
    return await _get_cart_or_404(CartRepository(session), cart_id)


@router.post("/{cart_id}/items", response_model=Cart)
async def add_item(cart_id: str, body: AddItemRequest, session: SessionDep) -> Cart:
    cart_repo = CartRepository(session)
    await _get_cart_or_404(cart_repo, cart_id)
    ...
    return await cart_repo.add_item(
        cart_id, sku=body.sku, quantity=body.quantity, unit_price=product.price
    )


@router.delete("/{cart_id}/items/{sku}", response_model=Cart)
async def remove_item(cart_id: str, sku: str, session: SessionDep) -> Cart:
    cart_repo = CartRepository(session)
    await _get_cart_or_404(cart_repo, cart_id)
    return await cart_repo.remove_item(cart_id, sku)
